...
- **Simulation semantics** (`simulate()` in `src/simulation.js`): the schedule is `ratePeriods` — per-line `{months, rate, fee}` fixed periods; each fee is rolled into the balance at the start of its period, and the last line's months/rate repeat fee-free until payoff (recalculation boundaries keep that cadence). The lender's *contractual payment* stays fixed within each period and is recalculated only at rate changes; how it's recalculated depends on the overpayment mode — `reduceTerm` keeps the payment level so overpayments shorten the loan (via `impliedRemainingTerm`), `reducePayment` re-amortizes to the original end date. The overpayment is the user's fixed payment minus the contractual payment, capped at 10% of the balance as it stood at the start of each year; payments below the contractual payment are raised to it and flagged. The final payment is clamped so the balance lands on exactly zero. An optional `fullRepaymentMonth` clears the remaining balance as a lump sum at a fix boundary — exempt from the 10% cap (no early repayment charge at the end of a fix) and not counted in total overpayments. An optional time-limited `extraPayment` (still cap-subject) models redirected standalone-loan payments for the debt-consolidation comparison; that comparison must keep cash flows matched — don't let the extra payment run past the standalone loan's term or the saving is overstated.
